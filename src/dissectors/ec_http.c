...
void __init http_init(void)
{
   ...
   sslw_dissect_add("insync", 6061, dissector_http, SSL_ENABLED);
...
FUNC_DECODER(dissector_http)
{
    ...
    */
   if (FROM_CLIENT("http", PACKET) || FROM_CLIENT("proxy", PACKET) || FROM_CLIENT("https", PACKET) || FROM_CLIENT("insync", PACKET)) {
      /* Check Proxy or WWW auth first
       * then password in the GET or POST.
       */
      char *p = memmem(ptr, PACKET->DATA.len, "auth.validatepwd", 16);
      if (p) { /* plaintext authentication is being used */
         p = memmem(ptr, PACKET->DATA.len, "\x00\x00\x00\x75", 4); /* find user name */
         if (p) {
            char tmp[MAX_ASCII_ADDR_LEN];
            int ulen = p[4];
            char username[256 + 1] = { 0 };
            strncpy(username, &p[8], ulen);
            username[256] = 0;
            char password[33] = { 0 };
            int plen = p[3 + 1 + 4 + ulen + 1];
            int index = 3 + 1 + 4 + ulen + 1 + 4;
            strncpy(password, &p[index], plen);
            password[32] = 0;
            DISSECT_MSG("%s:%d <= %s:%s\n", ip_addr_ntoa(&PACKET->L3.dst, tmp), ntohs(PACKET->L4.dst), username, password);
         }
      }
      p = memmem(ptr, PACKET->DATA.len, "authlite", 8);
      if (p) { /* authentication key is being used */
         p = memmem(ptr, PACKET->DATA.len, "\x00\x00\x00\x75", 4); /* find user name */
         if (p) {
            char tmp[MAX_ASCII_ADDR_LEN];
            int len = p[4];
            char username[256 + 1] = { 0 };
            strncpy(username, &p[8], len);
            username[256] = 0;
            char hash[33] = { 0 };
            int index = 3 + 1 + 4 + len + 1 + 4;
            strncpy(hash, &p[index], 32);
            hash[32] = 0;
            DISSECT_MSG("%s:%d <= %s:%s\n", ip_addr_ntoa(&PACKET->L3.dst, tmp), ntohs(PACKET->L4.dst), username, hash);
         }
      }
